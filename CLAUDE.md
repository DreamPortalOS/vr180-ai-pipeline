...
- **共享检出里禁止裸 `git commit`**。主检出 `D:\Githubr180-ai-pipeline` 可能同时有别的会话在工作，
  它们的 `git add` 会进同一个索引。09-08 一次裸提交把别人暂存的 8 个文件一起带进了我的 PR。
  提交一律带路径：`git commit -- <你改的文件>`；提交前先 `git status` 看一眼暂存区里有没有不属于你的东西。
  更稳的做法：**别在主检出里改东西**，用你自己的 worktree。
- **边界锁对"删除"同样生效**：`video/`、`models/`、`.venv/` 不许改，也**不许删**。
  要清理素材必须先在 PR 里列清单、由 lead 转交 owner 拍板，不得自行执行。
- **额度/网关 429 会毫无预兆掐断你**（09-07 一次掐掉三个并行执行者）。所以：**先推分支，再求完美**。
  写出第一个能自洽的提交后**立刻 `git push -u origin <branch>`**，之后每次 commit 都顺手 push。
  分支推上去了，接力的人能从你的进度继续；只躺在本地 worktree 里的提交，别人看不见。
